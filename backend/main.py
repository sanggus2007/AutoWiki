...
- `edges`의 label은 "**[A시작노드이름]**이/가 **[B도착노드이름]**의 [핵심 관계]" 형태로 작성하세요. (예: '**토니 스타크**가 **페퍼 포츠**의 배우자임', '**토니 스타크**가 **하워드 스타크**의 아들임')
- **관계 레이블 작성 절대 규칙**:
  1. 노드 이름 부분은 대괄호(`[...]`)나 소괄호(`(...)`) 등 어떠한 괄호 기호도 사용하지 말고, 오직 볼드 마크다운 기호(`**노드이름**`)로만 감싸야 합니다.
  2. '신뢰함', '미워함', '존경함', '갈등을 겪음' 등 주관적이거나 감정적이고 일시적인 성격의 관계 서술은 절대 금지합니다.
  3. '배우자임', '자녀임', '설립함', '소속됨', '개발함', '지배함'과 같이 제3자가 봐도 객관적이고 명확하며 건조한 백과사전식 핵심 관계(사실 기반)를 서술형으로 작성하세요.
...
{
  "plan_summary": "인공지능 및 앨런 튜링에 관한 새 문서를 생성하고, 중복된 기존 '양자 컴퓨팅' 문서를 삭제할 계획입니다.",
  "patches": [
  ...
  "edges": [
    {"source": "new-ai-doc", "target": "origin-wiki", "label": "**신규 인공지능 문서**가 **기존 문서**를 참조함"}
  ]
}
...
11. **철저한 사실 기반 작성 (환각 방지)**: 제공된 소스 문서나 참고 파일 등에서 확인되지 않은 내용은 절대 상상하여 작성하지 마세요. 특히 '그럴듯하게 들린다'는 이유만으로 지어내거나 추측하는 정보는 철저히 배제하고, 확실하게 검증된 백과사전식 사실관계만 담아 서술하십시오.
12. **인용구/어록 작성 절대 제한 규칙**:
    - 제공된 소스 텍스트나 프로젝트 파일에 **인물 혹은 캐릭터가 실제로 발언했다는 구체적인 기록(텍스트 상의 명확한 발언 증거)**이 존재하지 않는 한, 어떠한 명언이나 인용구(어록 서식)도 **절대 지어내거나 생성해서는 안 됩니다.**
    - 특히 아래와 같은 인용구 서식(blockquote)을 활용한 환각(Hallucination) 현상이 빈번하므로 주의하십시오:
      > "인용할 내용을 여기에 씁니다."
      > — [[인물명]]
      이 서식은 **실제 발언 흔적이 100% 명확히 문서에 기재되어 있을 때만 사용**할 수 있으며, 그렇지 않을 경우 이 서식 자체를 아예 문서에 포함하지 말아야 합니다.
...
> — 사티아 나델라, Microsoft CEO (주의: 이 예시는 실제 발명이 확인되었을 때만 사용 가능합니다)
...
7. **인용구/어록 작성 절대 제한 규칙**:
   - 제공된 소스 텍스트나 프로젝트 파일에 **인물 혹은 캐릭터가 실제로 발언했다는 구체적인 기록(텍스트 상의 명확한 발언 증거)**이 존재하지 않는 한, 어떠한 명언이나 인용구(어록 서식)도 **절대 지어내거나 생성해서는 안 됩니다.**
   - 특히 blockquote를 활용한 인용구(예: `> "..." \n > — [[인물명]]`)는 **실제 증거가 확실할 때만 유지하거나 생성**해야 합니다. 텍스트상 근거 없는 허구의 대화나 명언 기재는 환각(Hallucination) 오류이므로 절대 금지합니다.
